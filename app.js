...
async function downloadLatestUpdate(){
 ...
 try{
  ...
  const install=$("#installVerifiedUpdate");if(install){install.disabled=false;install.textContent=`Install ${j.latestTag||latestUpdateInfo?.latestTag||"Update"}`}
  updateBadge("VERIFIED","online");
  setUpdateStatus(`<strong>Verified download complete.</strong> ${esc(j.fileName)} • ${esc(j.sizeText)}<br><span class="muted">SHA-256: ${esc(j.sha256)}</span><br><span class="muted">Staged at: ${esc(j.path)}</span><div class="update-warning"><strong>Ready to install:</strong> the updater will close the companion, back up the current application folder, install ${esc(j.latestTag||latestUpdateInfo?.latestTag||"the verified release")}, preserve monitor-config.json, and restart the app.</div>`);
 ...
 finally{setTimeout(()=>barWrap?.classList.add("hidden"),1200);if(dl){dl.disabled=!(latestUpdateInfo&&latestUpdateInfo.updateAvailable);dl.textContent=latestUpdateInfo&&latestUpdateInfo.updateAvailable?`Download & Verify ${latestUpdateInfo.latestTag}`:"Download & Verify Update"}}
}
